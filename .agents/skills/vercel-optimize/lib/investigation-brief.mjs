...
export function summarizeDeepDiveFailures(deepDive) {
  ...
  const codes = uniq(failures.map(([, v]) => v?.code ?? v?.error ?? 'unknown'))
    .slice(0, 3)
    .join(' / ');
  return `${failures.length} of ${
    entries.length
  } deep-dive signals failed (${failedIds}${
    failures.length > 6 ? ', …' : ''
  }) — error: ${codes}.`;
}
...
export function buildBrief({
  ...
  if (candidate.disqualified) {
    lines.push(
      `- **⚠ Disqualifier present:** ${
        candidate.disqualifyReason ?? 'disqualified'
      }`
    );
  }
  lines.push('');
  lines.push(
    `**Gate question (the hypothesis you're verifying):** ${
      candidate.question ?? '(no question)'
    }`
  );
  lines.push('');
  ...
  if (failureNotice) {
    ...
    lines.push('>');
    lines.push(
      `> The base evidence below is still valid — \`o11ySignal=${
        candidate.o11ySignal ?? '(unset)'
      }\` came directly from the gate's broad-pass query and is unaffected. Investigate against that signal and any deep-dive keys that DID populate. Do not conflate "missing data" with "no bottleneck": if the data didn't come back, abstain on the missing dimensions, not on the candidate as a whole.`
    );
    lines.push('');
  }
  lines.push(
