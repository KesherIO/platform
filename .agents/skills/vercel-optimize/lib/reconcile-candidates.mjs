...
function scannerOnlyDecision(candidate, ctx) {
  ...
  return dropWithObservation(candidate, ctx, {
    ...
    observation: {
      ...
      summary: `${targetLabel(
        candidate
      )} has a static scanner finding, but no route-level Vercel metric signal.`,
...
function slowRouteMetricMismatchDecision(candidate, ctx) {
  ...
  return dropWithObservation(candidate, ctx, {
    ...
    reason: `Deep-dive p95 (${formatMs(
      p95
    )}) is below the slow-route threshold, so the broad gate did not survive follow-up verification.`,
    observation: {
      kind: 'metric_mismatch',
      summary: `${targetLabel(
        candidate
      )} was flagged as slow in the broad pass, but follow-up p95 is below threshold.`,
      evidence: `${
        candidate.o11ySignal ?? 'gate signal unavailable'
      }; deepDive.latency.p95=${formatMs(p95)}`,
      suggestedAction:
...
function slowRouteErrorDecision(candidate, ctx) {
  ...
  return dropWithObservation(candidate, ctx, {
    ...
    reason: `Function-level 5xx responses dominate this route (${formatPct(
      rate
    )}), so this is a reliability finding rather than a slow-route finding.`,
    observation: {
      kind: 'error_storm',
      summary: `${targetLabel(
        candidate
      )} latency is dominated by function-level 5xx responses.`,
      evidence: `deepDive.statusDistribution: ${formatInteger(
        errors
      )} 5xx of ${formatInteger(total)} function invocations (${formatPct(
        rate
      )})`,
      suggestedAction:
...
function deploymentRegressionDecision(candidate, ctx) {
  ...
  return dropWithObservation(candidate, ctx, {
    ...
    reason: `One deployment is a large latency outlier (${
      worst.deploymentId
    } at ${formatMs(
      worst.p95
    )}), so the next action is regression triage rather than generic code optimization.`,
    observation: {
      kind: 'deployment_regression',
      summary: `${targetLabel(
        candidate
      )} p95 is concentrated in one deployment.`,
      evidence: `${worst.deploymentId} p95=${formatMs(
        worst.p95
      )} vs next highest ${second.deploymentId} p95=${formatMs(second.p95)}`,
      suggestedAction:
...
function routeErrorsMetricMismatchDecision(candidate, ctx) {
  ...
  return dropWithObservation(candidate, ctx, {
    reasonCode: 'metric_mismatch',
    reason: `Deep-dive 5xx volume (${formatInteger(
      confirmed5xx
    )}) does not confirm the broad route_errors gate (${formatInteger(
      broadErrors
    )}).`,
    observation: {
      kind: 'metric_mismatch',
      summary: `${targetLabel(
        candidate
      )} was flagged for 5xx errors, but follow-up status data did not confirm the volume.`,
      evidence: `${
        candidate.o11ySignal ?? 'gate signal unavailable'
      }; deepDive.confirmed5xx=${formatInteger(confirmed5xx)}`,
      suggestedAction:
...
function uncachedRouteCacheDecision(candidate, ctx) {
  ...
  return dropWithObservation(candidate, ctx, {
    reasonCode: 'metric_mismatch',
    reason: `Deep-dive cache hit rate (${formatPct(
      hitRate
    )}) is already healthy, so the uncached-route gate did not survive follow-up verification.`,
    observation: {
      kind: 'metric_mismatch',
      summary: `${targetLabel(
        candidate
      )} was flagged as low-cache, but follow-up cache data is already healthy.`,
      evidence: `${
        candidate.o11ySignal ?? 'gate signal unavailable'
      }; deepDive.cacheHitRate=${formatPct(hitRate)}`,
      suggestedAction:
...
function uncachedRouteMethodDecision(candidate, ctx) {
  ...
  return dropWithObservation(candidate, ctx, {
    ...
    reason: `Deep-dive GET share (${formatPct(
      getShare
    )}) is below the cacheable-route floor, so this is not a good shared-cache candidate.`,
    observation: {
      kind: 'protocol_mismatch',
      summary: `${targetLabel(
        candidate
      )} is not GET-heavy enough for a shared-cache recommendation.`,
      evidence: `${
        candidate.o11ySignal ?? 'gate signal unavailable'
      }; deepDive.getShare=${formatPct(getShare)}`,
      suggestedAction:
...
function isrOverrevalidationDecision(candidate, ctx) {
  ...
  if (reads <= 0) {
    return dropWithObservation(candidate, ctx, {
      ...
      observation: {
        ...
        summary: `${targetLabel(
          candidate
        )} was flagged for ISR over-revalidation, but follow-up ISR read data was empty.`,
        evidence: `${
          candidate.o11ySignal ?? 'gate signal unavailable'
        }; deepDive.isrWrites=${formatInteger(
          writes
        )}; deepDive.isrReads=${formatInteger(reads)}`,
        suggestedAction:
  ...
  return dropWithObservation(candidate, ctx, {
    ...
    observation: {
      ...
      summary: `${targetLabel(
        candidate
      )} was flagged for ISR over-revalidation, but follow-up ISR data did not confirm it.`,
      evidence: `${
        candidate.o11ySignal ?? 'gate signal unavailable'
      }; deepDive.isrWrites=${formatInteger(
        writes
      )}; deepDive.isrReads=${formatInteger(reads)}; ratio=${ratioLabel}`,
      suggestedAction:
