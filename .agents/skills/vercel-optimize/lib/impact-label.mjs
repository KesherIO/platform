...
export function synthesizeImpactFromSignal(rec, signals = {}) {
  ...
  if (cwvIssues.length > 0) {
    return `${tier} impact — bring ${joinEnglish(
      cwvIssues.map(formatCwvIssue)
    )}.`;
  }
  if (errors != null && errorRatePct != null) {
    ...
    return `${tier} impact — cut 5xx rate by ~${reductionPct}% to get below ${RELIABILITY_TARGET_PCT}% (current ${errorRatePct}%, ${formatInteger(
      errors
    )} errors in this window).`;
  }
  if (errors != null) {
    return `${tier} impact — resolve ${formatInteger(
      errors
    )} billed 5xx errors in this window.`;
  }
  if (cachePct != null && inv != null) {
    if (cachePct < CACHE_HIT_THRESHOLD_PCT) {
      return `${tier} impact — current cache hit rate is ${cachePct}% across ${formatInteger(
        inv
      )} requests in this window; the gate fires below ${CACHE_HIT_THRESHOLD_PCT}%.`;
    }
    if (p95 == null) {
      return `${tier} impact — current cache hit rate is ${cachePct}% across ${formatInteger(
        inv
      )} requests in this window; this recommendation targets the remaining uncached traffic.`;
    }
  }
  if (writes != null && reads != null) {
    const ratio = reads > 0 ? writes / reads : null;
    return ratio == null
      ? `${tier} impact — ${formatInteger(
          writes
        )} ISR write units with no recorded read units in this window.`
      : `${tier} impact — ${formatInteger(
          writes
        )} ISR write units vs ${formatInteger(
          reads
        )} read units in this window (${round2(ratio)} writes per read).`;
  }
  if (p95 != null && inv != null) {
    const multiple = p95 / SLOW_ROUTE_P95_THRESHOLD_MS;
    return `${tier} impact — current 95th percentile duration is ${formatInteger(
      p95
    )}ms across ${formatInteger(
      inv
    )} function invocations in this window (${round1(
      multiple
    )}x the ${formatInteger(
      SLOW_ROUTE_P95_THRESHOLD_MS
    )}ms slow-route threshold).`;
  }
  if (coldPct != null) {
...
function formatCwvIssue(i) {
  ...
  if (i.unit === 'ms') {
    return `${i.metric} below ${formatInteger(
      i.threshold
    )}ms (current ${formatInteger(current)}ms)`;
  }
