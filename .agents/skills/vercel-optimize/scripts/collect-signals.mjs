...
async function main() {
  ...
  log(
    `project link resolved (source=${project.source}; teamScope=${
      project.orgId ? 'yes' : 'no'
    })`
  );
  ...
  log(
    `framework=${stack.framework}@${stack.frameworkVersion ?? '?'} support=${
  ...
  log(
    `command scope resolved (source=${commandScope.source}; scoped=${
      scope ? 'yes' : 'no'
    })`
  );
  ...
  log(
    `observabilityPlusPreflight=${
      observabilityPlusConfig.access === true
        ? 'enabled'
        : observabilityPlusConfig.blocker ?? 'unknown'
    } (${observabilityPlusConfig.source})`
  );
  ...
  if (oplus) {
    ...
    if (!metricsCanaryOk) {
      ...
      log(
        `metrics access check failed: ${
          canary?.code ?? 'unknown'
        } — skipping full metrics fan-out`
      );
    } else {
  ...
  if (usageResult?.ok) {
    ...
    if (
      ...
      if (filterResult.matched) {
        ...
        log(
          `usage: filtered to project — ~$${usageTotalCost.toFixed(
            2
          )} (team-wide ~$${usageTeamTotal.toFixed(
            2
          )}; unattributed ~$${filterResult.unattributedTotal.toFixed(2)})`
        );
      } else {
        usageTotalCost = usageTeamTotal;
        log(
          `usage: ~$${usageTotalCost.toFixed(
            2
          )} billed across services (team-wide — no per-project usage rows matched the linked project; report will label this team-wide)`
        );
      }
    }
  } else {
    log(
      ...
      }) — degrading to scanner+metrics-only mode`
    );
  }
  ...
  log(
    `stack: ${stack.framework}@${stack.frameworkVersion ?? '?'} ${
      stack.hasAppRouter ? 'app-router' : ''
    }${stack.hasPagesRouter ? ' pages-router' : ''}${
      stack.orm !== 'none' ? ` orm=${stack.orm}` : ''
    }`
  );
...
function validateProjectScope(projectCfg, project) {
  if (!projectCfg || projectCfg.error) {
    return {
      ok: false,
      source: 'project-api',
      detail: `The resolved account could not read the resolved project (project API error=${
        projectCfg?.error ?? 'unknown'
      }).`,
    };
  }
