# Laboratory Sample-Processing and Result-Entry Workflow — Final Plan (v3.1)

**Date:** 2026-08-11
**Status:** Awaiting approval before implementation
**v3.1 amendments:** Template versioning redesign, ResultReportTest provenance, OrderedTestSource relation, MISSING_RESULT_TEMPLATE blocking, platform-to-lab template mapping via code convention
...
LabTestSpecimenRequirement (NEW — separate from LabTestConfiguration)
  ...
  ├── isAlternative: Boolean (false = required, true = acceptable alternative)
  ├── notes: String?
  └── sortOrder: Int
...
OrderedTestSource (NEW — preserves all ordering/billing origins)
  ...
  └── @@unique([orderedTestId, originCatalogItemId])
...
| 2 | `RECEIVED_BY_LAB` | All expected specimens are accounted for (ACCEPTED or explicitly REJECTED with tests re-routed/cancelled) AND no test has progressed past READY |
...
**Step 4 is optional for MVP.** Tests without templates can still be created in the catalog and ordered by clinics, but at accessioning time they will be BLOCKED with `MISSING_RESULT_TEMPLATE` until a template is published. The template readiness inventory (section 5) highlights gaps. If a lab wants to prevent ordering unconfigured tests entirely, they can deactivate the CatalogItem (`active = false`) until configuration is complete.
...
Formula evaluation is frontend-only (backend stores the string, does not evaluate). The result entry form must implement a formula parser that resolves `[CODE]` references from sibling analyte values and computes the result.
...
`LabTestSpecimenRequirement` is a separate table with a 1:N relationship to `LabTestConfiguration`. This allows:
...
LabTestConfiguration (1) ──→ LabTestSpecimenRequirement (N)
  Example:
  ├── Req 1: EDTA_BLOOD in EDTA_TUBE, 2mL, primary (isAlternative=false)
  └── Req 2: CITRATE_BLOOD in CITRATE_TUBE, 1.5mL, alternative (isAlternative=true)
```

When `isAlternative = true`, the specimen is an acceptable substitute for the primary. During accessioning, if the primary specimen type is unavailable, an alternative can be accepted. Tests requiring ANY of the listed specimen types are satisfied when at least one matching specimen is accepted.
...
**Late-arriving specimens:** If a specimen was EXPECTED but not received during initial accessioning, it can be added later. When a late specimen is accepted, its linked tests transition BLOCKED → READY.
...
6. Tests with `isAlternative=true` requirements are listed under their primary group but noted as having alternatives.
...
- **New `OrderedTestSource` model** (replaces `sourcePackageId`) — records direct and package origins per test
...
**Backend changes:**
- New Prisma models: `Specimen`, `OrderedTestSpecimen`
- New enum: `SpecimenStatus`
...
- `ResultEntryForm` component — the core deliverable:
  ...
  - Formula parser: evaluates `[CODE]` references from sibling analyte values in real-time
