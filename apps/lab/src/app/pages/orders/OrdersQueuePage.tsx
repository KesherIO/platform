...
import { LabAssistantPanel } from '../../shared/components/LabAssistantPanel';
...
export function OrdersQueuePage() {
  ...
  return (
    <div className="flex h-full">
          ...
          <div className="flex items-center gap-4">
            ...
            {!isAssistantOpen && (
              <button
                ...
                className="rounded-lg border border-gray-800 px-3 py-1.5 text-sm text-gray-300 transition hover:border-cyan/40 hover:text-cyan"
              >
                ✨ {t('assistant.toggle')}
              </button>
            )}
      ...
      {isAssistantOpen && (
        <LabAssistantPanel onClose={() => setIsAssistantOpen(false)} />
      )}
    </div>
  );
}
